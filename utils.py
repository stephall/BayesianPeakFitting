...
# Remark:
# The chapters here are the same as in the jupyter notebook 'DoublePeakFitting.ipynb', so 
# that one can more easily link helper functions (here) and analysis code (jupyter notebook).

####################################################################################
###### 0) GENERATE ARTIFICIAL DATA
####################################################################################
def fwhm_to_sigma(fwhm):
    """ Map Gaussian Full Width at Half Maximum (FWHM) to Gaussian sigma. """
...
####################################################################################
def get_model_samples(q, trace, discard=0):
    """"
    Generate the model samples for a given input of q values 'q'.
    
    Args:
        q (1d-array): Values of q at which we want to obtain the model samples at.
        trace (trace object): Trace object returned from pm.sample() that contains the parameter samples.
    ...
    Returns:
        The samples matrix with shape (#samples, #q)
    """
    # Make the input q 1d-array a 2d-array
    q = q.reshape(1, -1)

    # Get the parameter samples
    mu_1_samples      = trace.get_values('mu_1', burn=discard, combine=True).reshape(-1, 1)
    mu_2_samples      = trace.get_values('mu_2', burn=discard, combine=True).reshape(-1, 1)
    fwhm_1_samples    = trace.get_values('fwhm_1', burn=discard, combine=True).reshape(-1, 1)
    fwhm_2_samples    = trace.get_values('fwhm_2', burn=discard, combine=True).reshape(-1, 1)
    sum_int_samples   = trace.get_values('sum_int', burn=discard, combine=True).reshape(-1, 1)
    rel_int_1_samples = trace.get_values('rel_int_1', burn=discard, combine=True).reshape(-1, 1)
    bg_scale_samples  = trace.get_values('bg_scale', burn=discard, combine=True).reshape(-1, 1)
    bg_slope_samples  = trace.get_values('bg_slope', burn=discard, combine=True).reshape(-1, 1)

    # Determine some parameter samples from these
    rel_int_2_samples = 1.0 - rel_int_1_samples
    int_1_samples     = sum_int_samples*rel_int_1_samples
    int_2_samples     = sum_int_samples*rel_int_2_samples

    
    # Initialize y with contribution from first peak (using scipy.stats.norm.pdf for the peak shape)
    y_samples = int_1_samples*stats.norm.pdf(q, loc=mu_1_samples, scale=fwhm_to_sigma(fwhm_1_samples))
        
    # Add contribution from second peak (using scipy.stats.norm.pdf for the peak shape)
    y_samples += int_2_samples*stats.norm.pdf(q, loc=mu_2_samples, scale=fwhm_to_sigma(fwhm_2_samples))
    ...
    y_samples += bg_scale_samples*(1+bg_slope_samples*q)
